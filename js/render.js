...
var Render = (function () {
  ...
  function renderSection1() {
    ...
    // プログレスドット
    html += '<div class="q1-dots" id="q1-dots">';
    for (var d = 0; d < total; d++) {
      html += '<div class="q1-dot' + (d === 0 ? ' current' : '') +
        '" id="q1-dot-' + d + '" onclick="App.showQ1(' + d + ')"></div>';
    }
    html += '</div>';
    ...
    App.shiwakeQuestions.forEach(function (q, i) {
      ...
      html += '<div class="q1-page' + (i === 0 ? ' active' : '') +
        '" id="q1-page-' + i + '">';
      html += '<div class="card">';
      html += '<div class="card-header">';
      html += '<h3><span class="q-number">' + (i + 1) + '</span> 仕訳問題</h3>';
      html += '<span style="color:var(--gray-500);font-size:0.85rem">3点</span>';
      html += '</div>';
      ...
      // 借方
      html += '<div class="journal-side debit"><h4>借方（左側）</h4>';
      for (var r = 0; r < maxRows; r++) {
        html += buildPickerRow('q1_' + i + '_d_acc_' + r, 'q1_' + i + '_d_amt_' + r, q.choices);
      }
      html += '</div>';

      // 貸方
      html += '<div class="journal-side credit"><h4>貸方（右側）</h4>';
      for (var r = 0; r < maxRows; r++) {
        html += buildPickerRow('q1_' + i + '_c_acc_' + r, 'q1_' + i + '_c_amt_' + r, q.choices);
      }
      html += '</div>';
    ...
    // ナビゲーション
    html += '<div class="q1-nav">';
    html += '<button class="btn btn-outline" id="q1-prev" onclick="App.prevQ1()" disabled>&larr; 前へ</button>';
    html += '<span class="q1-counter" id="q1-counter">1 / ' + total + '</span>';
    if (total > 1) {
      html += '<button class="btn btn-primary" id="q1-next" onclick="App.nextQ1()">次へ &rarr;</button>';
    } else {
      html += '<span></span>';
    }
    html += '</div>';
    ...
    App.currentQ1Index = 0;
  }

  // 1行分のピッカー（科目選択＋金額入力）
  function buildPickerRow(accId, amtId, choices) {
    var html = '<div class="journal-row">';

    // 隠しinput（grading.js が読む）
    html += '<input type="hidden" id="' + accId + '" value="">';

    // タップで開く科目選択ボタン
    html += '<button type="button" class="account-picker-btn" id="' + accId +
      '_btn" onclick="App.togglePicker(\'' + accId + '\')">';
    html += '科目を選択 &#9660;';
    html += '</button>';

    // 選択肢メニュー（初期非表示）
    html += '<div class="account-picker-menu hidden" id="' + accId + '_menu">';
    choices.forEach(function (c, idx) {
      html += '<button type="button" class="picker-option" onclick="App.pickAccount(\'' +
        accId + '\', \'' + c.replace(/'/g, "\\'") + '\')">';
      html += CHOICE_LABELS[idx] + '. ' + c;
      html += '</button>';
    });
    html += '<button type="button" class="picker-option picker-clear" onclick="App.pickAccount(\'' +
      accId + '\', \'\')">';
    html += '&times; クリア';
    html += '</button>';
    html += '</div>';
    ...
    // 金額入力
    html += '<input type="number" id="' + amtId +
      '" placeholder="金額" class="amount-input" inputmode="numeric" oninput="App.updateQ1Dots()">';
  ...
  function renderSection3() {
    ...
    html += '</tbody></table></div>' +
      ...
      '<div style="padding:14px;background:var(--gray-50);border-radius:var(--radius);margin-bottom:20px">';
    ...
    kd.questions.forEach(function (q) {
      html += '<div class="q3-answer-row" id="' + q.id + '_wrap">' +
        '<label>' + q.label + ' <span class="q3-points">(' + q.points + '点)</span></label>' +
        '<div class="q3-input-group"><span>¥</span>' +
        '<input type="number" id="' + q.id + '" placeholder="金額を入力" inputmode="numeric">' +
        '</div>' +
        '</div>';
    });
  ...
  function renderQ1Details(q1) {
    ...
    q1.forEach(function (d, i) {
      ...
      html += '<div class="detail-item" style="border-color:' + borderColor + ';background:' + bgColor + '">' +
